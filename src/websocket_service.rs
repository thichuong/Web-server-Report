    ...
    async fn update_dashboard_data(
        ...
        match redis_client.get_multiplexed_async_connection().await {
            Ok(mut redis_conn) => {
                ...
                let _: () = redis_conn.set(REDIS_KEY_DASHBOARD, &summary_json).await?;
                let _: () = redis_conn.expire(REDIS_KEY_DASHBOARD, CACHE_TTL_SECONDS as i64).await?;
                    ...
                    Ok(Some(data)) => {
                        match serde_json::from_str(&data) {
                            ...
                            Err(e) => {
                                ...
                                let _: Result<(), _> = redis_conn.del(REDIS_KEY_DASHBOARD).await;
                                Ok(None)
                            }
                        }
                    },
